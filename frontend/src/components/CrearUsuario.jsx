...
const CrearUsuario = () => {
  ...
  const guardarDatos = async (e) => {
    ...
    try {
      ...
      await axios.post('https://proyecto-mern-2-a9zx.onrender.com/api/usuarios', formData, {
        headers: {
  ...
  const actualizarUser = async (e) => {
    ...
    try {
      // Enviar datos al backend
      await axios.put(`https://proyecto-mern-2.onrender.com/api/usuarios/${subId}`, formData, {
        headers: {
          'Content-Type': 'multipart/form-data' // Especificar el tipo de contenido
        }
      });
  ...
  const obtUno = async (id) => { // Función para obtener un usuario y mostrar la información en el formulario
    try {
      const res = await axios.get(`https://proyecto-mern-2-a9zx.onrender.com/api/usuarios/${id}`);
      setUsuario({
